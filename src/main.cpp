...
int main(int argc, char *argv[]) {
  ...
  ctts::eSpeakPhonemeConfig eSpeakConfig;
  ...
  if (runConfig.phonemeType == eSpeakPhonemes) {
    ...
    runConfig.textToPhonemes =
        [&eSpeakConfig](std::string text,
          ...
          ctts::phonemize_eSpeak(text, eSpeakConfig, phonemes);
        };
  } else {
    // Text "phonemes"
    if (ctts::DEFAULT_ALPHABET.count(runConfig.language) < 1) {
      throw std::runtime_error("Language is not supported for text phonemes");
    }

    idConfig.phonemeIdMap = std::make_shared<ctts::PhonemeIdMap>(
        ctts::DEFAULT_ALPHABET[runConfig.language]);
  }
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (!lineObj.contains("phonemes_ids")) {
      ...
      for (auto &sentencePhonemes : phonemes) {
        ...
        ctts::phonemes_to_ids(sentencePhonemes, idConfig, sentIds,
                               missingPhonemes);
